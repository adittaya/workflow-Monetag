...
- [x] **REAL ANDROID IN CI — KVM IS AVAILABLE (SHELVED)**: probe run proved `ubuntu-latest` now exposes **`/dev/kvm`** (VT-x, 4 cores, 15GB RAM, 88GB disk). docker-android image `budtmo/docker-android-x86-9.0` no longer exists; QEMU+KVM route (`apt install qemu-system-x86-64 qemu-utils android-tools-adb` + Android-x86 `android-x86_64-9.0-r2.iso` 965MB SourceForge + `hostfwd=tcp::5555-:5555` adb over network) was mid-probe when **user shelved the experiment to keep the normal desktop pipeline** — `android-probe.yml` deleted. **Not pursuing real-Android-in-CI for now; normal loop (desktop, 2 views/run) is the live path.** adb not preinstalled on runner.
